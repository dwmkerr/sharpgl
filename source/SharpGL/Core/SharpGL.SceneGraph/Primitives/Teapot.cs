...
namespace SharpGL.SceneGraph.Primitives
{
        ...
        IHasMaterial
    {
        ...
        public Teapot()
        {
            ...
            BoundingVolume.FromSphericalVolume(new Vertex(), 1.75f);
        }
        ...
        public void Draw(OpenGL gl, int grid, double scale, uint type)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                gl.Map2(OpenGL.GL_MAP2_TEXTURE_COORD_2, 0.0f, 1.0f, 2, 2, 0.0f, 1.0f, 4, 2, tex.Flatten());
                gl.Map2(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, p.Flatten());
                gl.MapGrid2(grid, 0.0, 1.0, grid, 0.0, 1.0);
                gl.EvalMesh2(type, 0, grid, 0, grid);
                gl.Map2(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, q.Flatten());
                gl.EvalMesh2(type, 0, grid, 0, grid);

                if (i < 6)
                {
                    gl.Map2(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, r.Flatten());
                    gl.EvalMesh2(type, 0, grid, 0, grid);
                    gl.Map2(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, s.Flatten());
                    gl.EvalMesh2(type, 0, grid, 0, grid);
                }
            }
