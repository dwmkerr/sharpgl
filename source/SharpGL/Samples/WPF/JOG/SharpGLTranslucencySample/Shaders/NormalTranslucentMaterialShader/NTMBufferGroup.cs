...
namespace SharpGLTranslucencySample
{
    public class NTMBufferGroup
    {
        ...
        #region constructors
        public NTMBufferGroup(OpenGL gl)
        {
            CreateBufferIds(gl);
        }
            ...
            uint[] indices, float[] vertices, float[] normals, Material[] materials)
        {
            ...
            float[] ambs = new float[4 * materials.Length],
            ...
            for (int i = 0; i < materials.Length; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ambs[i * 4 + j] = mat.Ambient[j + 1];
                    diffs[newPos] = mat.Diffuse[j + 1];
                    specs[newPos] = mat.Specular[j + 1];
                    newPos++;
                }
                ambs[i * 4 + 3] = mat.Ambient[0]; // Add alpha channel to ambient.
